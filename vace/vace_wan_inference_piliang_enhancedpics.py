...
def main(args):
    ...
    prompt_json = json.load(open("./prompt_withfacedescription.json"))
    
    video_list = os.listdir("/root/autodl-tmp/vip200k_test_track_Facial")
    ret_data = {}
    for video in tqdm.tqdm(prompt_json):
        if '.' in video:
            continue
        ...
        for prompt_id in prompt_json[video]:
            ...
            args.src_ref_images = "/root/autodl-tmp/VACE-main/output_picenhancedbyace/"+video+"_"+prompt_id+".png"
            ...
            args.prompt = prompt_json[video][prompt_id]
        
            
            # from IPython import embed;embed()
            save_file = "./output_vanilla_test_idpromptimage_enhanced/"+video+'_'+str(prompt_id)+".mp4"
            video_list = os.listdir("./output_vanilla_test_idpromptimage_enhanced/")
