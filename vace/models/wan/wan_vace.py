...
@contextmanager
def patch_model_forward(model, block_indices, patch_method):
    """临时替换模型中指定 block 的 forward 方法，退出时自动恢复"""
    # 保存原始方法
    original_forwards = {}
    for idx in block_indices:
        ...
        # 保存原始方法供 patch_method 内部调用
        block.original_forward = block.forward
        
        # 替换为新方法
        block.forward = types.MethodType(patch_method, block)
    ...
    try:
        ...
        # 恢复所有方法，无论是否发生异常
        for idx in block_indices:
            model.blocks[idx].forward = original_forwards[idx]
            # 清理临时属性
            if hasattr(model.blocks[idx], 'original_forward'):
                del model.blocks[idx].original_forward
...
# Copied from diffusers.models.transformers.transformer_wan.py
...
class WanVace(WanT2V):
    ...
    def generate(self,
        ...
        if negative_prompt is not None:
            context_neg = self.text_encoder([negative_prompt], torch.device('cpu'))
        ...
        no_id_z0 = self.vace_encode_frames(input_frames, [None], masks=input_masks)
        no_id_m0 = self.vace_encode_masks(input_masks, [None])
        no_id_z = self.vace_latent(no_id_z0, no_id_m0)
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            arg_neg = {'context': context_neg, 'seq_len': seq_len}
            ...
            for _, t in enumerate(tqdm(timesteps)):
                ...
                skip_guidance_scale=0.5
                
                with patch_model_forward(self.model, [2], forward_with_stg):
                    # 执行模型，此时 blocks[2] 使用替换后的方法
                    # if _==1:
                    #     arg_neg["judge"] = True

                    noise_perturb = self.model(
                        latent_model_input, t=timestep, vace_context=no_id_z, vace_context_scale=context_scale,  **arg_c
                    )[0]
                    # from IPython import embed;embed()
                    # 计算最终结果
                    noise_pred = noise_pred + skip_guidance_scale * (noise_pred - noise_perturb)
