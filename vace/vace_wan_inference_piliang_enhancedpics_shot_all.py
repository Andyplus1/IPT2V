...
def get_parser():
    ...
    parser.add_argument(
        "--ckpt_dir",
        type=str,
        default='/root/autodl-tmp/Wan2.1-VACE-14B/',
...
def main(args):
    ...
    prompt_json = json.load(open("./prompt_withfacedescription_all.json"))
    negative_prompt = "Bright tones, overexposed, static, blurred details, subtitles, style, works, paintings, images, static, overall gray, worst quality, low quality, JPEG compression residue, ugly, incomplete, extra fingers, poorly drawn hands, poorly drawn faces, deformed, disfigured, misshapen limbs, fused fingers, still picture, messy background, three legs, many people in the background, walking backwards"

    video_list = os.listdir("/root/autodl-tmp/VACE-main-stg/test_50")
    ret_data = {}
    for video in tqdm.tqdm(video_list):
        if '.' in video:
            continue
        video_prompt_list = os.listdir("/root/autodl-tmp/VACE-main-stg/test_50/"+video)
        
        # for i in video_prompt_list:
        #     if '.txt' in i:
        #         prompt_num = i[-5]
        
        
        
        # for i in video_image_list:
        #     if '.webp' in i:
        #         temp = i
        #         break
        #     else:
        #         continue
        # convert_webp_to_image(video_image_dir+temp,"./temp.png")
        
        
        # for prompt_id in prompt_json[video]:
            # from IPython import embed;embed()
        args.src_ref_images = "/root/autodl-tmp/VACE-main-stg/test_50/"+video+"/image.png"
        ...
        # args.prompt = video_description.replace('\n','').replace('.','. ')
        args.prompt = prompt_json[video][str(prompt_num)]
    
        video_name = video+"_"+str(prompt_num)+".mp4"
        if video_name in os.listdir("./output_vanilla_test_idpromptimage_enhanced_stg_enhanced_all"):
            continue
        
        save_file = "./output_vanilla_test_idpromptimage_enhanced_stg_enhanced_all/"+video+"_"+str(prompt_num)+".mp4"
        # video_list = os.listdir("./output_vanilla_test_idpromptimage_enhanced_shot/")
        # if video+'_'+str(prompt_id)+".mp4" in video_list:
        #     continue
        # from IPython import embed;embed()
